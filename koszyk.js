...
app = new Vue({
	...
	methods: {
		purchaseClicked : function(){
			...
			jsonPostParams.email = sessionStorage.getItem("userEmail");

			for(var i = 0; i < this.cart.length; i++){
				jsonPostParams.produkty.push(this.cart[i].item_id);
				jsonPostParams.ilosc.push(this.cart[i].item_count);
			}

			if (jsonPostParams.produkty.length == 0){
				this.$toast.open({
			...
			axios.post('http://localhost:8080/warehouse/klient/zamowienie', jsonPostParams)
			.then((response) =>{
				this.$toast.open({
